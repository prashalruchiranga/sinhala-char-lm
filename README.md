...
Run `sample.py` to sample from the trained model. Text is generated by iteratively running the trained model for a number of steps specified by the `length` parameter. You can control the generation with `string` and `temperature` arguments to get different results.
```
